use crate::{
    ...
    parse_jwt_macro,
    Response, ResponseResult, ID, SEA_MAX_DAY, SEA_MIN_DAY,
};
use axum::{
    extract::Path, http::HeaderMap, routing::{get, post}, Json, Router
};
...
pub fn sea_router() -> Router {
    Router::new()
        ...
        .route("/sea/push/:id", post(push_customer_to_sea))
...
async fn push_customer_to_sea(headers: HeaderMap, Path(id): Path<String>) -> ResponseResult {
    ...
    //     "select c.name, ex.salesman from customer c 
